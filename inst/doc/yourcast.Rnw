...
\title{\YourCast: Software for Simultaneous Time Series Forecasting
  with Your Assumptions\thanks{Available from
    http://GKing.Harvard.Edu/yourcast.  For expert programming
    assistance and many helpful comments, our thanks goes to Jon
    Bishof, Elena Villalon, and Nirmala Ravishankar.}}

\author{Federico Girosi\thanks{Senior Policy Researcher, RAND
    Corporation, (1700 Main Street, PO Box 2138, Santa Monica, CA
    90407-2138; \texttt{http://www.ai.mit.edu/people/girosi/},
    \texttt{girosi@rand.org}, (310) 393-0411 x7794)} \and
%
...
output. The argument \code{dvlabel} gives a title for the plots by
describing the dependent variable. The second argument,
\code{age.insamp.predict}, tells the function not to plot predicted
\sQuote{yhat} values in sample in the \code{age} plots. You can see
...
Since we did not specify which type of plot we wanted, the program
gave us the default combination of \code{age} and \code{time}
plots. However, the plotting function can also do either of these
plots separately, as well as three dimensional plots. To see these, we
need to use the \code{family} argument. For example, here is a call
...
than printing it to the device window. Just set \code{print="pdf"}.
...
\include{Rd/yourcast}
\include{Rd/yourprep}
\include{Rd/plot.yourcast}
\include{Rd/histograph}
\include{summary.yourcast}
